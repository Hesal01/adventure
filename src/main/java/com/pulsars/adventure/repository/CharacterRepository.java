...
public interface CharacterRepository extends CrudRepository<Character, Long> {
    ...
    @Query("select c from Character c join fetch c.items i join fetch i.item it where c.user.id = ?1 and c.name = ?2 ")
    Character getCharacterByUserIdAndCharacterName(Long userId, String characterName);
